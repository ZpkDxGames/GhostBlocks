...
public class GhostBlockManager {
    ...
    private static final int MAX_AGE_RESET_PER_TICK = 20; // Reduced from 50 to 20 for better TPS optimization
    ...
    public void createGhostBlock(Location location, Material material) {
        ...
        // Use deprecated method for Paper 1.19.4 compatibility
        @SuppressWarnings("deprecation")
        FallingBlock fallingBlock = blockLocation.getWorld().spawnFallingBlock(blockLocation, material, (byte) 0);
        ...
        try {
            ...
            for (Map<?, ?> blockData : ghostBlocksList) {
                try {
                    ...
                    // Use deprecated method for Paper 1.19.4 compatibility
                    @SuppressWarnings("deprecation")
                    FallingBlock fallingBlock = world.spawnFallingBlock(blockLocation, material, (byte) 0);
                    fallingBlock.setGravity(false);
                    fallingBlock.setDropItem(false);
                    fallingBlock.setHurtEntities(false);
                    fallingBlock.setVelocity(new Vector(0, 0, 0));
    ...
    /**
     * Optimized age reset method that limits operations per tick with better distribution
     */
    public void resetGhostBlockAges() {
        ...
        // Process blocks in smaller batches to maintain TPS
        List<GhostBlock> blockList = new ArrayList<>(allBlocks);
        processBatchOfBlocks(blockList, 0);
    }
    
    /**
     * Process a batch of ghost blocks for age reset
     */
    private void processBatchOfBlocks(List<GhostBlock> blocks, int startIndex) {
        int endIndex = Math.min(startIndex + MAX_AGE_RESET_PER_TICK, blocks.size());
        
        for (int i = startIndex; i < endIndex; i++) {
            GhostBlock ghostBlock = blocks.get(i);
            if (ghostBlock.isValid()) {
                try {
        ...
        // If there are more blocks to process, schedule next batch for next tick
        if (endIndex < blocks.size()) {
            final int nextStart = endIndex;
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                processBatchOfBlocks(blocks, nextStart);
            }, 1L);
        }
    }
}
